...
-> 有图片就先转成结构化需求
-> 推断项目画像
-> 生成 project-docs/01_ARCHITECTURE.md 架构建议
-> 只反问会影响架构方向的关键问题
-> 用户确认后把架构状态改为 approved
-> 运行或确认 agentflow architecture check
-> 根据已确认架构提出 YAML 配置建议
-> 识别可能要导入的第三方模块
-> 询问用户配置和模块决策是否符合预期
-> 拆模块和里程碑
...
在初始 feature 表之前，Manager 应该先完成项目级架构确认。它不应该一开始反问一堆泛泛问题，而是先根据需求写出推荐架构，把不确定点放进 `Open Questions`，只追问会影响架构方向的问题，例如“是否必须私有化部署？”“是否已有指定技术栈？”“权限是否需要多租户隔离？”。

如果 feature 表里有不确定项，Manager 应该逐项反问，例如“这个功能是否必须支持移动端？”“这个模块是否涉及权限控制？”，而不是要求用户自己改表格。

需求导入后，Manager 应该自动做一次架构和配置评估，不需要用户另外说“帮我配置 YAML”。例如需求里出现用户管理、权限、支付、上传、移动端等信息时，Manager 应主动提出：
...
如果架构仍是 `draft`、包含 `TBD`，或还没有人工确认，Manager 应先补齐并确认架构，再创建初始 feature bundle。给已有项目临时创建单个小 feature 时，CLI 只会提示架构状态，不会强制阻断。
